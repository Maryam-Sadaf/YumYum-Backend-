    ...
    public async signUp({ request, response }: HttpContextContract) {
        try {
            ...
            if(restaurant.name !== null){
                ...
                return response.send(Response({ message: 'Already Exists Enter Valid License Key' }))
            }
            await restaurant.merge(data).save()
            return response.send(Response({ message: 'Restaurant SignUp Successfully' }))
        } catch (error) {
            console.log(error);
    ...
    public async login({ auth, request, response }: HttpContextContract) {
        try {
            ...
            if (restaurant?.licenseKey !== license_key) {
                ...
                return response.send(Response({ message: 'Invalid License Key' }))
            }
